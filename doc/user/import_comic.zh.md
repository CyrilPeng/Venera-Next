...
页面按文件名自然排序，例如 `图片_1.jpg`、`图片_2.jpg`、`图片_10.jpg`；支持多段数字、前导零和大小写文件名。
...
- 在 `本地` -> `导入` -> `PDF 漫画文件` 中可一次选择一个或多个 PDF；每个文件分别导入为一本漫画，收藏夹只需选择一次。
- 多个文件会按队列依次转换，界面同时显示当前文件、本批次文件数及当前文件的页数进度，无需逐本等待并重新选择。
- **主分支新增（未发布）**：点击“后台运行”、关闭进度框或返回即可继续阅读其他漫画，导入不会因此取消。可从主页“本地”卡片或本地漫画页的 PDF 图标进入“PDF 导入任务”，重新查看进度、取消任务或查看逐项结果；重新打开不会重复执行导入。再次选择 PDF 会加入同一个串行队列，等待中的任务可直接取消。已完成记录可清除，不会删除已导入漫画。
- 每本成功后立即保存到本地漫画库。重复选择的文件和已存在的同名漫画会跳过；单本损坏、加密或读取失败不会阻断后续文件，结束后可查看逐项结果。
- 点击取消会停止后续导入，并在当前文件读取或页面处理安全结束后清理未完成的漫画；已成功导入的漫画会保留，未处理文件不会计为失败。
- 导入期间请保持应用运行，不支持退出应用后继续任务或跨重启续传。Android 上无法直接访问的文件会逐本复制到临时目录，处理后释放。
...
- 普通目录的页面和章节按文件名自然排序；元数据明确指定的章节范围继续保留。
